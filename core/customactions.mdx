...
<Note>
  **Prompt Example:**
  ...
  - Use tool #Check_Lead_Status to retrieve caller details using the phone number.
</Note>
...
4. **Save** the action.
5. In the **Agent Settings**, go to **Actions → On Call → Custom Actions** and select the actions your agent should use during the call.
6. **Link it in your agent prompt** using the format **#ActionName**.
